...
@section('content')
    <div class="container pt-3">
        <div class="d-flex justify-content-between align-items-center">
            <h4>Edit Blok Perumahan</h4>
            <a href="/admin/blok-perumahan" class="btn btn-secondary">Kembali</a>
        </div>
        <hr>
        <main class="mb-3 row justify-content-center">
            <div class="col-12 col-md-6">
                @if ($errors->any())
                    ...
                    @endforeach
                @endif
                <form action="/admin/blok-perumahan/{{ $blok_perumahan->id }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="mb-3">
                        <label for="nama" class="form-label">Nama Blok Perumahan</label>
                        <input type="text" class="form-control" id="nama" name="nama" required autocomplete="off"
                            value="{{ old('nama', $blok_perumahan->nama) }}">
                    </div>
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" id="nomor_awal_rumah" name="nomor_awal_rumah"
                            ...
                            value="{{ old('nomor_awal_rumah', $blok_perumahan->nomor_awal_rumah) }}">
                    </div>
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control" id="nomor_akhir_rumah" name="nomor_akhir_rumah"
                            min="1" required autocomplete="off"
                            value="{{ old('nomor_akhir_rumah', $blok_perumahan->nomor_akhir_rumah) }}">
                    </div>
                    <div class="mb-3">
                        <label for="urutan" class="form-label">Urutan</label>
                        <input type="number" class="form-control" id="urutan" name="urutan" min="1" required
                            autocomplete="off" value="{{ old('urutan', $blok_perumahan->urutan) }}">
                    </div>
